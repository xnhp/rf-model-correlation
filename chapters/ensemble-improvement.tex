...
$$
Due to Lemma (\ref{qstars-same}) which states that in homogeneous ensembles $q_{i}^\star = q_{j}^\star = \bar{q}^\star$, we can conclude that $\Delta = 0$. This shows two things: First, the ensemble bias is equal to the average member bias:
$$
\Delta = 0 ~ ~ \rightarrow ~ ~ \LE{y}{\bar{q}^\star} = \Mavg \LE{y}{q_{i}^\star}
$$
This in turn implies that \textit{diversity is a component of ensemble variance}. The other component is the average member variance. In other words, \textit{variance reduction is measured exactly by diversity}.
...
% !! TODO that's not quite right, W_Theta was for an ensemble...?
\begin{definition} 
