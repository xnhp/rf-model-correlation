...
\marginnote{
    \textit{Recap}: $P$ is the parent node before the split with combiner $y_P$. $L$ and $R$ are the two child nodes after a split with combiners $y_L$ and $y_R$. 
}
...
% TODO
\sidenote{TODO explain, at least with intuition. Convexity of generator etc.}
...
If $p$ is a class probability distribution and $u \defeq [\frac{1}{k}, \dots, \frac{1}{k}]^\top$ a uniform distribution, then $p-u$ is a margin vector and the optimisation corresponds to maximising the classification margins as measured by the squared error.
